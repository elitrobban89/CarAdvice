...
627 tester täcker backendens rena logik och HTTP-lagret (beroenden mockas med Mockito; `FeedbackServiceTest` och `IceConsumptionServiceTest` kör mot H2 in-memory för att verifiera portabel SQL):

| Testklass | Täcker |
|-----------|--------|
| `GroqServiceTest` (185) | **Drivmedlet skickas med i `fuelSpec.fuel`** (2026-08-14). Frontenden räknade ägandekostnad med **förbrukningen som drivmedelsproxy** — `consumptionLiterPerMil > 7` gav dieselpris. En Kia Sportage 1.6 T-GDI drar 8,0 l/100 km och prissattes därför som **diesel**, medan en snål diesel under 7 fick bensinpris: fel åt båda hållen. Värdet fanns hela tiden i `ice_consumption`-raden som förbrukningssiffran hämtas ur, det skickades bara inte med. Fältet är **alltid verifierat** — AI:ns egen gissning nollställs först och fylls bara av DB-raden, samma linje som förbrukning och hk. Utan DB-träff lämnas det `null` och frontenden (`caIsDiesel`) faller tillbaka på den gamla tröskeln, som är det bästa som finns där. **Växellådefältet städas från motorbeteckningar** (2026-08-14). Ett skarpt sök gav kortet "Volvo XC40 (2022)" växellådan *"Automat 8-växlad (TSI turbo)"* — **TSI är VW-koncernens beteckning**, bilen var en Volvo B4, och samma bil hade i en tidigare körning korrekt "Automat Geartronic 8-växlad". Att strängen varierade mellan körningar visade att den kom från AI:n och inte från databasen. **Roten satt i promptens eget exempel:** `"gearbox":"Automat DSG 7-växlad (TSI turbo)"` plus instruktionen "ange turbo/ej turbo" — modellen fyllde i den parentes prompten bad om och tog motorfamiljen ur exemplet. Samma mönster som MG5-fallet: **modellen agerar på namngivna exempel**, så ett exempel med ett märkesspecifikt ord blir en mall som följer med till fel märke. Exemplet är utbytt och beteckningarna uttryckligen förbjudna, men regeln står på **två ställen** av samma skäl som utmärkelseskärpningen: en prompt kan ignoreras. Till skillnad från förbrukning, hk och motoralternativ går växellådan **inte** att verifiera mot `ice_consumption`, som inte lagrar den — därför städning i stället för ersättning. En parentes behålls bara när den innehåller ett känt växellådeord: "Automat (CVT)" är vettigt, "(TSI turbo)" inte. Okänt innehåll faller bort, och det är rätt håll att fela åt — kvar står alltid själva växellådan, och motorn visas ändå verifierad i `engineOptions` och `horsepower`. **`ice_consumption` har företräde också i KORTBYGGET** (2026-08-14). Skarpt sök på SUV/bensin/250 000 kr gav korten "Kia Niro (2021)" och "Hyundai Kona (2020)" en elbils `evSpec` — *ladda var 10:e dag* respektive *var 6:e dag* — samtidigt som deras `fuelSpec` korrekt visade bensinmotorn (6,2 och 6,8 l per mil). Samma symtom som C-HR-buggen dagen innan men via en helt annan väg: här är namnmatchningen **inte** fel, `Kia Niro` finns faktiskt som elbil och radens ord är en äkta övermängd av titelns, så ingen strängregel i `EvSpecService` kan skilja dem åt. **Företrädesregeln fanns redan men satt på fel ställe:** `isNonEv` har sedan 2026-08-09 regeln att `ice_consumption` prövas före `ev_spec`, men den användes bara av drivmedelsvakten — alltså för att avgöra om ett *elbilssök* fått riktiga elbilar. Kortbygget hämtade `evSpec` för varje titel oavsett vad användaren sökt på, med en årsmodellkoll som enda filter. Tre saker behåller sin `evSpec` eftersom de faktiskt har en sladd, och testet låser alla tre: titeln säger själv `ev`/`phev`; förbränningsträffen är en **laddhybrid** (`Volvo XC60 T8` ska visa sin elräckvidd); eller titeln bär ett märkesnamn för den laddbara varianten som inte är ett generellt drivlineord (`Recharge`, `E-Tense`, `4xe`, `E-Tech`) — utan dem hade ett äkta elbils-XC40 tappat sina chips, eftersom XC40 också finns som bensinbil. Listan får bara växa åt det hållet: en glömd post kostar ett tomt fält på ett elbilskort, motsatsen ger laddråd på en bensinbil. Fail open vid DB-fel. **Kategorin `elbil` är lika bindande som `laddhybrid` i `fuelIntent`** (2026-08-13). Formuläret döljer drivmedelsrutan för båda kategorierna och tvingar värdet till "spelar ingen roll" — en sträng som bär delsträngen "el" **inuti "spelar"**. Laddhybrid räddades av sin `carCategory`-gren, elbil hade ingen: `ev` och `ice` blev båda sanna och `pureEv()` **falskt**. Formulärets elbilssök — appens vanligaste elbilsväg — slog därför ut **alla fem** konsumenter av villkoret samtidigt: `requirePureEvCars` kördes aldrig, "ELBIL OBLIGATORISKT" saknades i prompten, **båda** nypristabellerna följde med (det tyngsta promptfallet, samma storleksklass som gav HTTP 413), `activeConstraints` utelämnade "ren elbil" ur kravlistan användaren får se, och `affordableModelsLine` lämnade kandidatlistan tom. Den sista är den allvarligaste: listan byggdes 2026-08-10 just för att golvvakten fällde EV6/Ioniq 5/Enyaq utan att omförsöket fyllde på till tre kort, alltså var botemedlet avstängt på exakt den väg symtomet uppstod. Alla fem satt i samma villkor, så fixen sitter i `fuelIntent` och inte hos anroparna. Live-verifieringarna missade felet för att de gick via API med `fuelType="el"`, där `pureEv()` redan var sant — formulärets egen payload provades aldrig, och testet gör nu just det. **Drivmedel och växellåda översätts till Blockets fältvärden** (`adFilterFor`): "Hybrid bensin" räknas som bensin men "Plug-in Bensin" inte — en självladdande hybrid är en bensinbil, en laddhybrid är något annat än det användaren bad om. Laddhybridsbrasklappen (`PHEV_TAX_CAVEAT`: triggas på **kategorin** laddhybrid eftersom drivmedelsrutan inte har det valet, säger uttryckligen att begagnat inte påverkas, och stänger av BEV-tvånget så en kvarglömd "el"-drivmedelsrad inte förbjuder det användaren bett om; övriga kategorier får ingen brasklapp) och drivmedelsklassningen (**"Hybrid (ej laddhybrid)" är en bensinbil**, inte en elbil — den matchade tidigare `wantsEv` på delsträngen "hybrid" och fick både BEV-tvång och en utelämnad ICE-nypristabell). Budgettaket (`exceedsBudgetCeiling`: bil 84 000 kr över budget faller, 25 000 kr över släpps igenom, gränsen går vid exakt +30 000, billig bil faller aldrig — taket har inget golv, en ensam annons fäller ingen bil, taket mäts mot billigaste annonsen och inte snittet) och sammanslagningen efter omförsöket (`mergeWithinBudget`: för dyr bil tas bort även när omförsöket inte blev bättre, omförsökets bilar först, aldrig fler än tre, tom lista när ingen håller budget, bil utan Blocket-data faller inte, **dedup på modell och inte titel** så ihopslagningen inte ger "ID.4 (2022)" bredvid "ID.4 (2021)"), promptbygget (budget/leasing, milprofil, laddbox, växellåda, ÅLDERSKRAV, familjeprofil-flaggning), systemprompt-reglerna (EV/ICE-pristabellfiltrering, exakt 3 bilar, fabricerade priser, småbilsförbud + kuraterade familjebilar, budgetutnyttjande, märkesprioritet, årsmodeller före lansering, SIFFERLOGIK/DRIVLINA i jämförelseprompten), familjespärren (`requireFamilySizedCars`, och att gränsen går vid **5+ passagerare** — den gick vid 4, vilket är formulärets defaultvärde, så strängaste läget var påslaget för alla som inte rörde fältet; ett elbilssök på 225 000 kr gav då två kort medan samma sökning med två passagerare gav tre, Zoe/Leaf/MG ZS EV, alltså just det billiga elbilsutbudet spärren höll ute. **VW ID.3 är borttagen ur småbilslistan**: Golf-klass med fem säten, samma storleksklass som MG4 som prompten samtidigt rekommenderar som familjeelbil), drivmedelsvakten (`requirePureEvCars`: hybrid i titeln fälls, förbränningsbil utan drivlineord i titeln fälls via `ice_consumption`, okänd bil släpps igenom — vakten kräver positivt bevis. **`ice_consumption` prövas före `ev_spec`**, annars släpps tvetydiga namn igenom: live 2026-08-09 gav ett elbilssök "Kia Niro (2021)", som finns som HEV, PHEV *och* elbil, och `ev_spec`:s fuzzy-matchning slog mot "Kia Niro EV" innan hybridträffen ens provades. Ett namn som finns som förbränning fälls därför även om det också finns som elbil — en titel utan drivlineord pekar inte ut någon variant, och tvetydigheten är skadlig i sig eftersom Blocket-uppslaget då matchar hybridannonser och kortet får fel prisbild; `fuelIntent` avgör när den gäller, med delsträngsfällorna "diesel" och "spelar ingen roll" som båda innehåller "el"; regelbrottet bär med sig de elbilar som fanns i svaret så att de kan visas när även omförsöket bröt mot regeln, och bara ett svar helt utan elbilar blir ett felmeddelande). **Alla tre regelvakter delar samma mönster** (`RuleViolationException` bär med sig det som klarade regeln): förut hade var och en sin egen hårda väg ut, så ett enda regelbrott bland tre bilar kostade hela svaret — live 2026-08-09 gav elbil + 225 000 kr + 5 passagerare HTTP 500 i tre försök av tre, dels på en overifierbar modell, dels på avhuggen JSON. Vakterna körs i kedja och den första som fäller avbryter resten, så `keepPassingCars` kör om kedjan på det som blev kvar tills den går ren (varje varv tar bort minst en bil, alltså terminerar loopen). Omförsöket får **4 500 tokens** mot ordinarie 3 000 — "AI-svaret blev ofullständigt" betyder att resonemanget åt upp budgeten innan JSON:en hann skrivas färdig, och samma tak upprepar oftast samma trunkering. Klarade **ingen** bil regeln görs ett **rättelseförsök** (`retryAfterRuleViolation`) som pekar ut de fällda bilarna vid namn och upprepar regeln som en instruktion — samma grepp som budgetomförsöket, och av samma skäl: omförsöket i `parseWithRetry` skickar bara om samma prompt till reservmodellen utan att säga vad som var fel, och får därför ofta samma fel igen. Live-fallet var elbil för 175 000 kr, som gav enbart hybrider i båda omgångarna och därmed HTTP 500 fast det finns gott om elbilar i det prisläget. Varje vakt bär därför både de fällda bilarnas namn och sin regel formulerad som instruktion (`avvisade()`, `rattelse()`). Går det ändå inte får felet med sig **vad som går att ändra** (`medRadOmKriterier`: högre budget, färre passagerare, annat drivmedel) i stället för bara "Försök igen" — orsaken skrivs inte om, den varierar och gissas inte, årsmodellvakten (`requireRealisticModelYears`: "Kia EV3 (2022)" avvisas eftersom EV3 lanserades 2024 — live-fall 2026-08-09 som modellvakten släppte igenom, den godkänner EV3 för att modellen finns i databasen och kontrollerar inte årtalet. `MODEL_LAUNCH_YEAR` är spegel av promptens årsmodellregel och innehåller **bara kontrollerade lanseringsår**; generationsspannen i `new_car_price` duger inte som källa — "Volkswagen Polo 2018-2021" hade gjort 2015 års Polo till en påhittad bil fast tabellen bara saknar äldre generationer. Modell utanför listan eller titel utan årtal släpps alltid igenom), modellhallucinationsvakten (`requireKnownModels`: påhittade modeller avvisas, trimvarianter i båda riktningarna godkänns, tom whitelist släpper igenom, diakritiknormalisering, källsammanslagning + enordsfiltrering i `buildKnownModelTokenSets`), Blocket-snappingen (`correctedPrice`, tröskel 2 annonser), verifierade motoralternativ ersätter AI:ns engineOptions-fritext när ev_spec-träff finns (annars behålls AI:ns text), verifierad systemeffekt (`getSystemPowerHk`) ersätter AI:ns hk-gissning för EV-modeller med känd korrekt siffra (annars behålls AI:ns värde), verifierad hk/motorbeteckning från ice_consumption ersätter AI:ns gissning för bensin-/diesel-/hybridbilar (topplevel-hk OCH fuelSpec.horsepower rättas tillsammans så kortet inte visar två olika hk-tal, engineOptions blir motorbeteckningen ur databasen med modellordet strippat), JSON-parsning av AI-svar (`<think>`-strippning, fallback-nycklar, root-array, okända fält, avhugget/feltypat JSON → begripliga fel), cachenyckel, de delade cachehjälparna (`isFresh` säger nej till saknad post och till post äldre än TTL:n på 4 h, `store` lägger in med färsk tidsstämpel), 429/felmeddelanden, feedback-kontexten (undvik-signal), modellhälsokollen (`missingModels`) |
| `EvSpecServiceTest` (73) | **`isKnownEv` bär själv företrädet `ice_consumption` före `ev_spec`** (2026-08-14). En namnträff räcker inte som svar på "är kortet en ren elbil?": "Hyundai Kona" och "Kia Niro" finns som både bensinbil och elbil, och matchningen svarar ja på den nakna titeln eftersom radens ord (`Kona Electric`) är en äkta övermängd av titelns. Kortet klassades då som ren elbil och **tappade sina förbränningsinsikter** — kamrems- och oljeråd som hör hemma just där. **Regeln bodde först hos anroparen, och det var för klent:** samma företräde fanns redan i `GroqService` på två ställen (drivmedelsvakten sedan 2026-08-09, kortbygget sedan `ef26242`) och insiktsfiltret fick sin egen kopia i `128d829`. Tre kopior är tre chanser att glida isär, och en fjärde anropare hade gått i samma fälla igen — den bor nu i metoden som besvarar frågan, och kopian hos anroparen är borttagen. Villkoret är avsiktligt `drivetrain != null` och inte `"ev"`: en PHEV-titel svarade sant före ändringen och ska fortsätta göra det, annars ändras kontraktet för anropare som inte bett om det. Den enda anroparen når hit bara när drivlineordet saknas, så företrädet biter exakt där skadan fanns. **Ingen `electric`-token, och det var ett aktivt val:** ett generellt krav på ordet mättes till 52 → 43 färre felträffar, men inventeringen av de 26 rader som bär det visade att **Dacia Spring** och **Alpine A290** bara finns som elbil — en ordlista hade tystat deras kort, samma gräns som för e-prefixen. Testet låser båda hållen plus fail open vid DB-fel. **Pass 1 är ordgränsbaserat, och passen är två i stället för tre** (2026-08-14). Delsträngsmatchningen i pass 1 var roten till en hel buggfamilj med sex kända ingångar: `"c-hr"` ligger inuti `"c-hr+"`, `"250"` inuti `"250+"`, `"tt"` inuti `"quattro"`, `"a"`/`"b"`/`"c"` inuti nästan varje Mercedes-rad. Varje ingång hade lagats med en ny token i `DRIVLINEORD` — en lista som måste växa varje gång tabellen får ett nytt namnmönster. **Mätt offline innan bytet** (398 ICE-namnplåtar × alla 553 ev_spec-rader genom fyra varianter, samma upplägg som tröskelmätningen för `d9ff37b`): delsträng gav **67** namnplåtar som nådde en elbilsrad, ordgräns **52**, och delsträng-med-minst-3-tecken 53 — den sista nästan lika bra men värdelös strukturellt, eftersom `"c-hr"` är fyra tecken och slipper igenom, alltså hade tokenberoendet bestått. **Tre av de fyra tokens som lades in samma dag blev överflödiga** (`c-hr+`, `250+`, `4matic+`): varianten kördes både med och utan dem och gav exakt samma 52. `recharge` står kvar, för den bär inte ett *ändrat* ord utan ett *extra* — raden `Volvo XC40 Recharge` innehåller hela titeln "Volvo XC40" som riktiga ord. **Gamla pass 3 är borttaget för att det var identiskt med nya pass 1.** **Årsstrippningen fick en `(?<!\w)`-vakt**: ev-databases tekniska uppdateringskod `(TU2025)` träffade regexen, som kapade `"2025)"` mitt i ordet och lämnade skräpordet `"(tu"` — varpå `Renault Scenic E-Tech EV60/EV87 170hp (TU2025)` inte kunde matcha ens sitt **eget** namn, och `modelYear` läste 2025 som annonsens årsmodell. Delsträngsmatchningen dolde felet; ordgränsen avslöjade det. **Gränsen som INTE går att lösa här är utskriven i testet:** en naken namnplåt når fortfarande raden när radens ord är en äkta övermängd av titelns — `Mercedes-Benz CLA` mot `CLA 200`, `Kia Niro` mot `Kia Niro EV`, `Ford Mustang` mot `Mach-E`, `Mini Cooper` mot `Cooper E`. Det är modeller som finns som både förbränning och el, och ingen strängregel kan skilja dem åt; det avgörs ett lager upp av drivmedelsvakten och av att `ice_consumption` prövas före `ev_spec`. **Streckvarianter normaliseras** (2026-08-14): AI:n skrev "Toyota C‑HR" med **U+2011** (icke-brytande bindestreck) i ett skarpt sök. Lagrade namn bär U+002D, så en sådan titel kunde **aldrig matcha någon rad alls** — ett tyst fel utan felutskrift där kortet bara föll tillbaka på AI:ns fritext. Exakt samma sorts fälla som de smala mellanslagen (U+00A0/U+202F/U+200B) som redan normaliserades, och den drabbar varje modell med bindestreck i namnet: C-HR, e-tron, Mach-E, I-Pace, ë-C4. U+2010–U+2015 och U+2212 mappas nu till vanligt bindestreck, på båda sidor av jämförelsen. Testet låser båda riktningarna: en äkta träff ska bli av (`Ford Mustang Mach‑E` med U+2011 och U+2013), och spärrarna ska fortsätta bita fast strecket är av annan sort. **"Recharge" var ett felaktigt undantag** (2026-08-14): motiveringen var riktig i sak — Volvo använder namnet för både elbil och laddhybrid, och våra två rader (`Volvo XC40 Recharge`, `Recharge Twin`) är båda 75 kWh-elbilar — men undantaget skyddade bara elbilskorten, inte bensinbilen. Samma skarpa sök gav kortet "Volvo XC40 (2022)" en elbils `evSpec` (75 kWh, 530 km, *ladda var 11:e dag*) samtidigt som dess `fuelSpec` korrekt visade bensinmotorn B4 197 hk / 0,8 l per mil — en elbils laddråd och en bensinbils förbrukning på samma kort. Priset är att ett äkta elbils-XC40 vars titel bara säger "Volvo XC40" tappar sina chips, samma avvägning som redan gäller e-Golf och PHEV-raderna: **fel data på en bensinbil är värre än utebliven data på en elbil.** **Plus-suffixet är en identitetsmarkör, inte ett trimnamn** (2026-08-14): ett SUV/bensin-sök på 250 000 kr gav kortet "Toyota C-HR (2021)" rådet *ladda var 13:e dag*. Blocket-matchningen och drivmedelsfiltret gjorde rätt — bilen **är** en hybrid-C-HR — men pass 1 i `matchByTitle` matchar titelns ord som **delsträngar i hela det lagrade namnet**, och `"c-hr"` är delsträng av `"c-hr+"`. Plustecknet är det enda som skiljer den eldrivna C-HR+ (54/72 kWh) från hybriden, så delsträngsmatchningen raderade precis det tecken som bar identiteten, och `daysLabel` räknades fram ur den felmatchade radens räckvidd. **Femte ingången till samma buggfamilj** efter PHEV-suffix, GTE/Plug-in, saknad generation och e-prefix — och lärdomen "inventera hela tabellen först" tillämpad direkt: 19 rader bär `+`, varav **åtta gick att nå från en namnplåt som också finns i `ice_consumption`** (C-HR+ ×2 mot C-HR 1.8/2.0 Hybrid, CLA 250+, GLA 250+, GLB 250+ och tre AMG GT 4-Door 4MATIC+). Mercedes-trion är värre än C-HR: en bensin-"GLA 250" träffar `GLA 250+` eftersom `"250"` är delsträng av `"250+"`. Bara `c-hr+`, `250+` och `4matic+` står i spärren — de tre vars basord också finns som förbränningsbil. Ett generellt "alla plus-tokens" hade varit fel åt andra hållet, och testet låser den gränsen: `Smart #1 Pro+`, `XPENG P7+`, `Geely EX5 Pro+` och `Nissan Leaf e+` är elbilar hela vägen, där **ska** en enkel titel hitta trimraden. Samma gräns som "Audi Q4 e-tron behöver inget skydd". **Hela `ev_spec` inventerad 2026-08-13** (539 rader) efter att ett skarpt sök gav "Tesla Model 3 (2020)" med Highland-varianter på 716 och 750 km. Två skilda defekter föll ut. **Klass A — bara nyaste generationen finns**, alltså samma form som Leaf/ZS EV/e-Golf: `Renault Zoe` (bara ZE50), `Tesla Model 3` (bara Highland), Model Y, Kona Electric, Polestar 2, ID.4, IONIQ 5 och bZ4X. **Suffix skyddar INTE en rad från fel kort.** Första inventeringen friade ID.3 och i3 med motiveringen att extra ord i radnamnet ("ID.3 **Neo** 50 kWh", "i3 **50 xDrive**") skulle hindra en match mot en kortare titel. Det var bakvänt: `verifiedEngineOptions` kräver att varje ord i **titeln** finns i **radens** namn, så en längre rad matchar alltid en kortare titel. Live 2026-08-13 gav ett kort för "Volkswagen ID.3 (2022)" Neo-varianter med 630 km. Exponeringslistan gjordes om mot rätt regel: 28 titlar drar in fler än en rad med minst en otaggad, men de flesta är trimnivåer inom en generation — ID.3 var det enda äkta nya fallet, och i3 (enda raden är 2026 års Neue Klasse, 108,7 kWh / 912 km) syns inte i en flerradslista alls. **Hela klassen är åtgärdad** — taggning hjälper inte där, det krävs nya rader, och samtliga värden är användbar kapacitet plus WLTP **TEH** (den strängare testprofilen) avlästa på ev-database, samma källa nattsynken använder. Bara volymvarianten per generation läggs till där bara en är belagd; entry- och Performance-utföranden utelämnas hellre än gissas, av samma skäl som priset lämnas 0. Undantagen är Kona och IONIQ 5, där båda batterierna var lika vanliga och båda finns belagda. **Konventionsglappet är känt:** faceliftraderna bär i flera fall bruttokapacitet (IONIQ 5 84, Model Y 79) medan de nya bär netto — källornas egen inkonsekvens, precis som Enyaqs iV-rader, och ofarlig eftersom `verifiedEngineOptions` bara slår ihop rader med identisk räckvidd. **Klass B — båda generationerna finns men otaggade**: `Kia EV6` var det ENDA fallet i hela tabellen, och skiljelinjen är batteriet (58/77,4 pre-facelift mot 63/84 facelift). Raderna utan kWh i namnet (60, 80) är nettoangivelser av samma paket som 63 respektive 84 — de har alltså samma generation som sin bruttotvilling, och testsviten fångade att första försöket lade 60 och 63 i olika generationer trots identisk räckvidd. **Generationen ingår i gruppidentiteten**, så två rader ur olika generationer slås aldrig ihop oavsett kWh-avstånd. **`e-`-prefixet strippas ur BÅDA sidor, med spärr** (2026-08-11): titeln har alltid strippat det ("e-Niro" → "Niro") men det lagrade namnet inte, så rader vars *modellord* bär prefixet var omöjliga att träffa med ordmatchning och fick aldrig verifierade motoralternativ — ~30 rader (e-Tourneo, e-Caravelle, e-Transporter, e-Rifter, e-Traveller). Symmetrin öppnar samtidigt en risk: "e-Golf" är inte "Golf", så de e-prefixade modellnamn vars basord också finns som förbränningsbil står i samma spärr som PHEV-raderna och kräver att titeln bär prefixet. `isKnownEv("Volkswagen Golf")` är alltså falskt även med e-Golf-raderna på plats. **Volkswagen e-Golf lades till** (24,2 kWh 2014–2016, 35,8 kWh från facelift 2017) — bilen saknades helt, så kortet föll tillbaka på AI:ns fritext som påstod "45 kWh", en kapacitet den aldrig haft. **Trimnamn som bara upprepar kWh-talet tas bort**: raderna för utgångna generationer särskiljs av kapaciteten i namnet, vilket gav "44.5 kWh (263 km) · 44.5 kWh". Ett trim som säger något mer behålls — "e+" är modellbeteckning, inte kapacitet. **MG ZS EV och Nissan Leaf finns som flera generationer** — se `DataLoader.UTGANGNA_GENERATIONER`; ZS EV:s enda rad var faceliftens Long Range (72,6 kWh / 440 km), så kortet "MG ZS EV (2020)" fick 2022 års bil i stället för pre-faceliftens 44,5 kWh / 263 km. Bara pre-facelift-raden läggs till av oss: faceliftens Standard Range finns hos ev-database och det som har en riktig källa ska synken äga. **Laddhybridmarkörerna är inventerade ur tabellen, inte gissade** (2026-08-11): första versionen tog bara `phev`/`hev`, generaliserat från de två exempel som fanns för handen, och verifieringssöket samma dag gav kortet "Volkswagen e-Golf (2019)" motoralternativet "13 kWh (70 km) · GTE" — e-prefixregeln gör titeln till "volkswagen golf", som matchar laddhybriden `Volkswagen Golf GTE`. Listan är nu `phev`/`hev`/`gte`/`plug-in`, vilket täcker Golf GTE, Passat GTE, Prius Plug-in och RAV4 Plug-in utöver de 21 PHEV-raderna. De två Plug-in-raderna är dubbletter av Prius/RAV4 PHEV under en annan konvention och raderas *inte* — nattsynken kan återskapa dem, och spärren gör dem ofarliga där de står. Testet låser också att filtret inte blir för brett: `Volvo XC40 Recharge` (Recharge är Volvos namn för både elbil och laddhybrid), `DS 3 E-Tense` och `Jeep Compass Electric 4xe` är riktiga elbilar och deras kort ska fungera. **Nissan Leaf finns som tre generationer** (2026-08-11): tabellen hade en enda Leaf-rad — 75,1 kWh / 62,4 mil, alltså 2026 års bil — så ett kort för "Nissan Leaf (2019)" visade den enda rad som fanns, och årsfiltret hade inget att välja mellan. Gen 1 (24/30 kWh) och gen 2 (40 kWh, e+ 62 kWh) ligger i `DataLoader.LEAF_UTGANGNA` och återställs vid varje uppstart, som `MG4_GEN1`, eftersom ev-database bara listar den nuvarande bilen. **Alla tre generationerna är taggade i `GENERATION`, inte bara de nya** — otaggad rad räknas som den äldsta, och här är den otaggade raden tvärtom den nyaste. **Räckviddskonventionen skiljer sig avsiktligt:** gen 2 har riktiga WLTP-tal (270 respektive 385 km), medan gen 1 såldes före WLTP och bara har NEDC (175/250 km) som är så optimistiska att de hade återskapat just det fel raderna lades in för att åtgärda — därför verklig bruksräckvidd, ~120 och ~150 km. Priset lämnas `0`: vi saknar källa på svenska listpriser, och `toDto` sätter prisvärdhetsetikett bara när priset är > 0, så en gissning hade blivit en felaktig etikett i stället för ett tomt fält. **Laddhybridrader får inte fastna på ett elbilskort** (2026-08-11): `ev_spec` bär 21 PHEV-rader vid sidan av elbilarna, och matchningen strippar "Electric" ur titeln för att "MG4 Electric" ska hitta "MG4" — varpå "Hyundai Kona Electric" blir "Hyundai Kona" och båda orden finns i "Hyundai Kona PHEV". Live gav det ett rent elbilskort motoralternativet "8.9 kWh (58 km) · PHEV", alltså en laddhybrids 5,8 mil på en bil som går 51. Värre för `isKnownEv`, som svarar insiktsfiltret på om kortet är en ren elbil: "Volvo XC60" matchade "Volvo XC60 PHEV", så en bensin-XC60 klassades som elbil och tappade sina förbränningsinsikter. Ett drivlineord i det lagrade namnet kräver nu samma ord i titeln — prövat mot titelns egna ord *före* strippningen, annars hade PHEV-kortet blivit av med sin egen rad. **Årsmodellen väljer generation också i `matchByTitle`**: filtret satt bara i `verifiedEngineOptions`, så motorlistan kunde visa rätt generation medan spec-chipsen bredvid visade den andra (en MG4 från 2023 fick gen 2:s siffror). Samma generationsdata styr nu båda, och rör inget när årsmodell saknas. Fuzzy-matchning AI-titel → EV-spec i övrigt: pass 1–3, normalisering av diakritiska tecken **och av hårda/smala/zero-width-mellanslag** (U+00A0/U+202F/U+200B — AI-titlar med sådana matchade tidigare ingenting alls), strippning av årsmodell/`Electric`/`e-`-prefix, räckvidds- och prisvärdhetsberäkningar, prisvärd räckvidd-rankningen (km/krona, 400 km-golv, etablerade märken), verifierade motor-/batterivarianter (`verifiedEngineOptions`: **ett kort per variant, inte per batteri** — MG4:s fyra rader blir "Standard Range / Long Range / XPOWER / Extended Range" och gen 2:s sex varianter sex rader. Bara dubbletter slås ihop, och räckvidden avgör vilket som är vilket: olika kWh + samma räckvidd är netto/brutto av samma bil (EV6 Standard Range 60/63 kWh på 428 km → en rad), samma kWh + olika räckvidd är två varianter (MG4:s 52,8 kWh på 405 respektive 416 km → två rader). Ihopslagningen söker i alla grupper, inte bara den intilliggande, eftersom sorteringen på kapacitet lägger EV6 Long Range 2WD:s 80- och 84-rader isär trots samma 582 km. Tydligt olika batterier (58/77 kWh) och två generationers batterier i samma modell (EV6 77,4/84 kWh) hålls isär, dubblettrader dedupas, variant utan räckvidd slås ihop som förut (inget finns att skilja den på), ingen match ger null, märkesprefixet i "MG MG4 XPOWER" skalas bort och en hopslagen rad lämnas omärkt), verifierad systemeffekt (`getSystemPowerHk`: MG Marvel R Standard/Performance ger 180/288hk mot AI:ns tidigare felaktiga 150hk, mest specifika modellnamnet vinner, okänd modell ger null). Trepassmatchningen är utbruten till `matchByTitle` och delas av `formatForTitle` och `isKnownEv` — den senare svarar på "är kortet en ren elbil?" åt insiktsfiltret, och en egen kopia hade kunnat glida isär från den bil kortet visar specar för |
...
| `AutoDataScraperServiceTest` (19) | Parsern mot auto-data.net, körd på sidor sparade 2026-08-12 — källan valdes efter att nio kandidater provats skarpt och är den enda som bär både bagagevolym och motorvarianter för förbränningsbilar (`bilweb.se`, `car.info` och `carfolio.com` visade ingen volym alls, `biluppgifter.se` har ingen modellväg, `ultimatespecs.com` lämnade fältet tomt). **Generationssidan är motoralternativlistan**: Golf VIII ger 21 varianter i ETT anrop, mot de 13 Golf-rader `ice-consumption.csv` bär. Varje rad har två länkar till samma variantsida (rubrik- och datacellen) så `th`-filtret prövas — utan det står varje motor två gånger på kortet. **Bagagevolymen sitter på variantsidan** och tas från `Trunk (boot) space - minimum`, alltså normalvolymen med baksätet uppfällt; maxvolymen (Golf 380 mot 1 237 l) hade gjort bagagekravet verkningslöst. Två fällor i samma cell är låsta: imperialkolumnen `<span class="val2">13.42 cu. ft.</span>` skalas bort (en girig siffermatchning ger annars 13 liter, ett tal som ser rimligt ut och är fel) och `Fuel tank capacity 50 l` tre rader ned förväxlas inte med bagaget. **Generationsvalet är samma buggfamilj som MG4/Leaf/ZS EV**: årsspannet filtrerar först (en 2012:a kan aldrig få Golf VIII), därefter karossen via titeln — "Golf VIII Variant" väljs bort för bilnamnet "Volkswagen Golf" men vinner för "Volkswagen Golf Variant", annars får en halvkombi kombins liter. Generationer överlappar alltid (Golf VII såldes till 2021, VIII kom 2020), så ett krav på entydighet hade gett null för nästan varje skarvår — den som börjar senast vinner, och mest specifika titeln före det så att Alltrack inte tas av basmodellen. Märkeslistan läses ur sidfoten, vilket sparar sitemapen (27 MB i fyra skärvor, sju språk) |
| `IceConsumptionServiceTest` (27) | **Titelns drivlineord slår sökningens drivmedelsval** (2026-08-14). Ett bensinsök gav kortet `Kia Sportage Hybrid (2020)` med 9,2 l/100 km och verifierat drivmedel "bensin". `fuelPref` är sökningens önskemål om hela träfflistan, men den filtrerade bort modellens hybridrader, och hk-valet tog den bensinrad som råkade ha samma effekt: `1.6 T-GDI 230 hk 4x4` (0,92 l/mil) i stället för `1.6 T-GDI HEV 230 hk` (0,68) — samma effekt, fel bil, 35 % fel förbrukning. Sedan `1662af8` bär kortet dessutom radens `fuel` som verifierat värde, så felet blev "bensin" på ett kort som heter Hybrid *och* bensinpris i ägandekostnaden. **Titeln säger vilken bil kortet visar, sökningen bara vad användaren letar efter** — därför vinner titeln. Ordlistan är `ExpertInsightService.drivetrainOf`, samma markörer som insiktsfiltret och `isKnownEv` redan dömer på, så "Plug-in Hybrid"/PHEV prövas före "Hybrid" och stavningen spelar ingen roll (Kia skriver `HEV`, Hyundai `Hybrid`). Gränsen åt andra hållet är låst i samma test: en titel utan drivlineord styrs av `fuelPref` precis som förut, och en titel som utlovar en drivlina modellen saknar hos oss (`Alfa Romeo Giulia Hybrid`) faller tillbaka på `fuelPref` i stället för att lämna kortet utan siffra. Plus: **Motorlistan är EN generation, och äldre årsmodeller får den inte** (2026-08-13). `ice_consumption` bär ingen årsmodell och `engineOptionsForTitle` kastade årtalet före matchningen, så **202 av 310 namnplåtar** visade sin lista oavsett vilket år kortet gällde. Mätningen som vände bilden: samtliga tretton Golf-rader är Golf VIII (2020–2024) — `1.0 eTSI Mild Hybrid`, `eHybrid PHEV`, `GTI Clubsport` — och Golf VII har **noll** rader hos oss. Listan blandade alltså inte generationer, den *var* en generation applicerad på alla år, precis som elbilarnas klass A. Vakten läser `ice_generation` (eget bord: `ice_consumption` har PK `(brand, variant)` och samma beteckning återkommer äkta mellan generationer) och returnerar **null** för ett äldre år, varpå kortet faller tillbaka på AI:ns egen motortext. Att i stället hämta de äldre varianterna okurerat från auto-data hade gett 46 Golf VII-rader varav flera (`1.5 TGI`, `1.4 TGI` — fordonsgas) aldrig sålts i Sverige: vår CSV är kurerad för svensk marknad och det är dess värde. Okänt generationsår och saknat årtal betyder "ingen åsikt" och visar listan som förut — tabellen fylls över flera nätter av 03:00-jobbet och ett kort får inte tappa sina motoralternativ under tiden. Nyckelformen är testad: `allModelNames()` ger märket som det står plus modellordet normaliserat, alltså `"Volkswagen golf"`, och uppslag och ifyllning måste använda exakt samma sträng. Plus sedan tidigare: seed från ice-consumption.csv (957 varianter), titelmatchning (märke+modell, hk-närmaste variant, drivmedelsfilter), jämförelsesammanfattning, hk-parsning, modellord som upprepar märkesnamnet i variant-strängen hoppas över (Mazda 3/CX-5, DS-alla — annars matchar t.ex. Mazda CX-5 på Mazda 3:ans motor), ordgränsmatchning istället för substräng (Mazda CX-30/CX-60 matchar inte Mazda 3:ans/6:ans modellord "3"/"6" bara för att de råkar vara substrängar av "cx-30"/"cx-60"), `engineDescriptor` (motorbeteckning utan modellnamn/märkesupprepning för engineOptions-kortet), och **hela motorutbudet som lista** (`engineOptionsForTitle`) — tabellen bär 957 varianter på 304 modeller och 197 av dem har fler än en (Volvo XC60: B5, B6, D4, D5, T6 PHEV, T8 PHEV), men kortet visade bara den hästkraftsnärmaste enda varianten medan elbilskorten alltid listat sina batterivarianter. Sorterat svagast först; drivmedelsfiltret är medvetet utelämnat (motoralternativ är vad bilen går att få med — förbrukningssiffran filtreras fortfarande), och modellordsskyddet gäller även här: en CX-30 får aldrig Mazda 3:ans Turbo 2.5 265 hk, men däremot sin egen Skyactiv-X 186 hk som båda bilarna faktiskt har — mot riktig H2 |
...
| `BlocketPriceServiceTest` (27) | Prisspannet på korten, och därmed talet budgettaket mäts mot — siffrorna i testet är mätta mot Blockets API, inte påhittade. **Prisgolvet var blint för drivmedel och växellåda till 2026-08-13** (`AdFilter`): sökningen skickar bara `q=<märke modell>` och filtrerade på pris, årsmodell, mil och modellsiffra — aldrig på `fuel` eller `transmission`, fast Blocket skickar med båda i varje annons. En förfrågan om bensin + automat fick alltså budgettaket mätt mot en manuell diesel. Uppmätt samma dag under 10 000 mil: Volvo XC60 golv 125 500 kr mot 249 900 för billigaste bensinautomat, XC40 2019 227 000 mot 259 500 (bilen slank under 230 000-taket med 3 000 kr marginal på annonser användaren inte kan köpa), och VW T-Roc golv 159 900 fast det fanns **noll** bensinautomater under 10 000 mil. Filtret har **samma trappa som milgränsen och av samma skäl**: ett för snävt filter vore inte en skärpning utan en tyst uppluckring, eftersom `exceedsBudgetCeiling` bara kan fälla en bil som har en prisrad — därför faller det tillbaka på ofiltrerat när träffarna understiger två, och villkoret är `count < 2` och inte bara `null` (en ensam träff avväpnar taket lika effektivt som noll). Tomt fältvärde räknas som icke-match, till skillnad från saknad körsträcka som släpps igenom: här är hela poängen att golvet ska sättas av en bil som bevisligen matchar. **Milgränsen** (`MAX_MILEAGE_MIL` = 10 000): utan den sätts golvet av marknadens mest slitna exemplar, och eftersom taket mäts mot billigaste annonsen blev bilar "köpbara" på en bil ingen tänkt sig — Enyaq visades från 229 900 kr på en 21 091-milare medan billigaste exemplaret under 10 000 mil kostade 339 900 kr. Gränsen **lättas i steg** (`MILEAGE_STEPS_MIL`: 10 000 → 15 000 → hela marknaden) i stället för att släppas: ett steg som ger färre än två annonser duger inte som prisrad, men att då ta in hela marknaden direkt återskapade precis det fel milgränsen finns för. Sista steget är kvar som sista utväg — utan prisrad kan `exceedsBudgetCeiling` inte fälla bilen alls, eftersom den kräver två annonser. Saknad körsträcka släpps igenom (att kasta okända hade strukit riktiga annonser den dagen Blocket slutar fylla fältet). **Leasing är ett eget prisläge**: kr/mån och köppris får aldrig dela spann (Blockets `sales_form` räcker inte som skiljelinje — leasingannonser låg märkta "begagnad till salu", så beloppets storlek avgör), årsfiltret är avstängt där (annonserna ligger på 2026–2027 medan AI:n sätter ett begagnat-årtal), och ett köppris i månadsfältet räknas inte som månadskostnad (en ID.4 låg som leasingannons med 539 500 kr). **Avbetalning och företagsleasing sätter inte golvet** (2026-08-12): Leasing-hinken bär även finansieringsannonser — annons 18117231 säger i texten "Köp på avbetalning 2723kr / månad eller leasing för företag 2358kr / månad" — och de är systematiskt billigast. API:t har inget filter som skiljer dem åt, men beloppet är momsuppdelat: `amount_ex_vat` är satt på företags- och avbetalningsannonser och saknas på privatleasing. Testet låser båda riktningarna, eftersom regeln annars kunde tömma leasingspannet. **Undre gränsen är billigaste annonsen**, inte 20:e percentilen (som kastade en femtedel av marknadens billigaste: Kia EV3 visades från 434 900 kr när billigaste annonsen låg på 369 900 kr); bara medianrelativa avvikare (0,4× och 2,5×) trimmas. Årsmodellen filtreras om på `docs[].year` eftersom API:ts egna årsparametrar redan en gång slutat fungera tyst, och fabricerad årsmodell ger inget pris alls i stället för hela modellens spann. Syskonmodeller med annan siffra räknas inte (`q=Volkswagen ID.4` gav ID. Buzz på 699 900 kr, som satte taket), medan namn utan siffror (Enyaq, Model Y, Leaf) inte filtreras alls |
